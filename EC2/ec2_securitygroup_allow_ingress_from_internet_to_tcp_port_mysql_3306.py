...
def check_security_group_ingress(ec2_client):
    ...
    for security_group in response['SecurityGroups']:
        report = {
            ...
            'status': "PASS",
            'detail': f"Security group {security_group['GroupName']} ({security_group['GroupId']}) does not have MySQL port 3306 open to the Internet."
        }
            ...
            if ingress_rule.get('FromPort') in check_ports and ingress_rule.get('ToPort') in check_ports:
                for ip_range in ip_ranges:
                    if ip_range.get('CidrIp') == '0.0.0.0/0':
                        report['status'] = "FAIL"
                        report['detail'] = f"Security group {security_group['GroupName']} ({security_group['GroupId']}) has MySQL port 3306 open to the Internet."
                        break
...
def save_findings_to_json(finding, filename):
    if finding:
        with open(filename, 'w', encoding='utf-8') as file:
            json.dump(finding, file, indent=4, ensure_ascii=False)
...
if __name__ == "__main__":
    ...
    save_findings_to_json(result, 'ec2_securitygroup_allow_ingress_from_internet_to_tcp_port_mysql_3306.json')
    # 결과를 JSON 형식으로 출력
    print(f"Results saved to 'ec2_securitygroup_allow_ingress_from_internet_to_tcp_port_mysql_3306.json'.")
