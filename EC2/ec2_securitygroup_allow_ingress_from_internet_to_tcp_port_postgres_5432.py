...
def check_security_group_ingress_for_postgres_port(ec2_client):
    check_ports = [5432]
    findings = []
    ...
    for security_group in response['SecurityGroups']:
        report = {
            ...
            'status': "PASS",
            'detail': f"Security group {security_group['GroupName']} ({security_group['GroupId']}) does not have Postgres port 5432 open to the Internet."
        }
            ...
            if ingress_rule.get('FromPort') in check_ports and ingress_rule.get('ToPort') in check_ports:
                for ip_range in ip_ranges:
                    if ip_range.get('CidrIp') == '0.0.0.0/0':  # Checking if the rule allows access from the Internet
                        report['status'] = "FAIL"
                        report['detail'] = f"Security group {security_group['GroupName']} ({security_group['GroupId']}) has Postgres port 5432 open to the Internet."
                        break
...
if __name__ == "__main__":
    ...
    # 함수 호출 및 결과 저장
    result = check_security_group_ingress_for_postgres_port(ec2_client)
    save_findings_to_json(result, 'ec2_securitygroup_allow_ingress_from_internet_to_tcp_port_postgres_5432.json')

    # 결과를 JSON 형식으로 출력
    print(f"Results saved to 'ec2_securitygroup_allow_ingress_from_internet_to_tcp_port_postgres_5432.json'.")
