...
def ec2_securitygroup_allow_ingress_from_internet_to_ssh_port_22(ec2_client):
    findings = []  # 결과를 저장할 리스트 초기화
    check_port = 22  # 검사할 SSH 포트
        ...
        for security_group in security_groups:
            ...
            if vpc_in_use.get(vpc_id, False) and len(network_interfaces) > 0:
                ...
                report = {
                    ...
                    "status_extended": f"Security group {security_group.get('GroupName', '')} ({security_group['GroupId']}) does not have SSH port 22 open to the Internet."  # 상태 확장 설명
                }
                ...
                for ingress_rule in security_group['IpPermissions']:
                    if (ingress_rule['IpProtocol'] == 'tcp' and
                        ...
                        report["status"] = "FAIL"
                        report["status_extended"] = f"Security group {security_group.get('GroupName', '')} ({security_group['GroupId']}) has SSH port 22 open to the Internet."
                        break
...
if __name__ == "__main__":
    ...
    # 함수 호출 및 결과 저장
    result = ec2_securitygroup_allow_ingress_from_internet_to_ssh_port_22(ec2_client)

    # 결과를 JSON 형식으로 출력
    print(json.dumps(result, indent=4))

    # 결과를 JSON 형식으로 파일에 저장하는 경우 (필요 시 주석 해제)
    # with open('ec2_securitygroup_allow_ingress_ssh_port_22_results.json', 'w') as json_file:
