...
def ec2_networkacl_allow_ingress_any_port2(ec2_client):
    ...
    try:
        ...
        for network_acl in network_acls:
            ...
            report = {
                ...
                "status_extended": f"Network ACL {network_acl['NetworkAclId']} does not have every port open to the Internet."  # 상태 확장 설명
            }

            # 네트워크 ACL의 Entry들을 확인하여 모든 포트가 인터넷에 열려 있는지 검사
            if check_network_acl(network_acl['Entries'], tcp_protocol, check_port):
                report["status"] = "FAIL"
                report["status_extended"] = f"Network ACL {network_acl['NetworkAclId']} has every port open to the Internet."
...
if __name__ == "__main__":
    ...
    result = ec2_networkacl_allow_ingress_any_port2(ec2_client)  # 함수 호출하여 결과 저장

    print(json.dumps(result, indent=4))  # 결과를 JSON 형식으로 출력

    # 결과를 JSON 형식으로 파일에 저장하는 경우 (필요 시 주석 해제)
    # with open('ec2_networkacl_allow_ingress_any_port_results.json', 'w') as json_file:
